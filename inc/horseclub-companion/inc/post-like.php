...
add_action( 'wp_enqueue_scripts', 'horseclub_sl_enqueue_scripts' );
function horseclub_sl_enqueue_scripts() {

	wp_enqueue_script( 'horseclub-post-likes', HORSECLUB_COMPANION_DIR_URL . 'js/post-likes.js', array( 'jquery' ), '0.5', false );

	wp_localize_script( 'horseclub-post-likes', 'simpleLikes', array(
		'ajaxurl' => admin_url( 'admin-ajax.php' ),
		'unlike'  => esc_html__( 'Unlike', 'horseclub' )
	) );
}
 ...
 * @since    0.5
 */
add_action( 'wp_ajax_nopriv_horseclub_process_simple_like', 'horseclub_process_simple_like' );
add_action( 'wp_ajax_horseclub_process_simple_like', 'horseclub_process_simple_like' );
function horseclub_process_simple_like() {
	// Horseclub
	$nonce = isset( $_REQUEST['nonce'] ) ? sanitize_text_field( $_REQUEST['nonce'] ) : 0;
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( esc_html__( 'Not permitted', 'horseclub' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	$output = '';
	$nonce = wp_create_nonce( 'simple-likes-nonce' ); // Horseclub
	if ( $is_comment == 1 ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = esc_html__( 'Unlike', 'horseclub' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = esc_html__( 'Like', 'horseclub' );
		$icon = $icon_empty;
	}
...
function get_like_count( $like_count ) {
	$like_text = esc_html__( '', 'horseclub' );
...
// User Profile List
add_action( 'show_user_profile', 'horseclub_show_user_likes' );
add_action( 'edit_user_profile', 'horseclub_show_user_likes' );
function horseclub_show_user_likes( $user ) { ?>        
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'horseclub' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php esc_html__( 'You do not like anything yet.', 'horseclub' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
